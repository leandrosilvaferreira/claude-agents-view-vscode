...
Developer documentation for **Claude & Antigravity Agent Monitor**: local setup, running the
...
`preLaunchTask`). Exercise the **Agent Monitor** view there against your own `~/.claude` /
`~/.gemini` logs.
